...
exports.ExpressDockerGenerator = void 0;
class ExpressDockerGenerator {
    generate(projectInfo, config = {}) {
        const finalConfig = {
        ...
        if (finalConfig.hasTypeScript) {
            dockerCommands.push('# Build TypeScript', 'RUN npm run build', '');
        }
        ...
        if (finalConfig.isDevelopment) {
            dockerCommands.push('# Development setup', 'RUN npm install --only=development', `ENV PORT=${finalConfig.port}`, `EXPOSE ${finalConfig.port}`, 'CMD ["npm", "run", "dev"]');
        }
        else {
            // Production setup
            dockerCommands.push('# Production setup', `ENV PORT=${finalConfig.port}`, `EXPOSE ${finalConfig.port}`, 'CMD ["npm", "start"]');
        }
        return dockerCommands.join('\n');
    }
    generateCompose(projectInfo, config = {}) {
...
exports.ExpressDockerGenerator = ExpressDockerGenerator;
